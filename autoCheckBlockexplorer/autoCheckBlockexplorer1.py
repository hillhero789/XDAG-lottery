...
startTime = datetime.datetime.strptime('2018-10-27 12:10:00.000', "%Y-%m-%d %H:%M:%S.%f")  #游戏开始 UTC 时间
...
while True:
    c.get('https://explorer.xdag.io/block/SNiOG7aUUyZ3QmSl87T0CsUezb5C5l5X')   #读取页面数据
    time.sleep(5)       #不延时会出问题

    tbody = c.find_elements_by_tag_name('tbody')    #tbody[2]为交易数据所在表格
    tds = tbody[0].find_elements_by_tag_name('td')  #tds[0].text 传输方向，tds[1].text 传输哈希，tds[2].text 金额，tds[3].text 时间
    lastTX = tds[1].text
    
    if  (not (lastTX == preLastTx)) and (float(tds[2].text) == lotteryPrice) :  #条件成立表示有新的有效交易产生
        preLastTx = lastTX
        lastTxTime = datetime.datetime.strptime(tds[3].text, "%Y-%m-%d %H:%M:%S.%f") 
